...
def stacking(image_matrix, scale):
    ...
    max_len = max([len(i) for i in image_matrix])  # max_len which stores the max len of row in image_matrix
    ...
    for i in same_dim_images_matrix:
        scaled_image_matrix.append([cv2.resize(j,(0,0), None, scale, scale) for j in i])
...
output = stacking(image_matrix, 0.5) # the images with all rows len are same
output2 = stacking(image_matrix1,0.5)  # the image which consists of blank images
